        ...
        <div class="max-w-full mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <form action="{{ route('dosen.jadwal.nilai.update', $jadwal->JadwalID) }}" method="POST">
                        @csrf
                        @method('PUT')
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    @forelse ($krs as $item)
                                        <tr class="hover:bg-gray-50">
                                            ...
                                            <td class="px-2 py-2 whitespace-nowrap text-sm">{{ $item->MhswID }}</td>
                                            <td class="px-6 py-2 whitespace-nowrap text-sm font-medium text-gray-900">{{ $item->NamaMahasiswa }}</td>
                                            ...
                                            <td class="px-2 py-2 whitespace-nowrap">
                                                <input type="number" step="0.01" name="nilai[{{ $item->KRSID }}][{{ $field }}]" 
                                                        value="{{ old('nilai.'.$item->KRSID.'.'.$field, $item->$field) }}"
                                            ...
                                            {{-- Hasil Kalkulasi --}}
                                            <td class="px-2 py-2 whitespace-nowrap text-sm text-center font-bold">{{ number_format($item->NilaiAkhir, 2) }}</td>
                                            <td class="px-2 py-2 whitespace-nowrap text-sm text-center font-bold">{{ $item->GradeNilai }}</td>
                                            <td class="px-2 py-2 whitespace-nowrap text-sm text-center font-bold">{{ number_format($item->BobotNilai, 2) }}</td>
                                        </tr>
                                    @empty
                                        <tr>
